package com.game.client.window.lwjgl.event;

import com.game.client.window.event.cursor.CursorPositionEvent;
import com.game.client.window.event.key.KeyActionType;
import com.game.client.window.event.key.KeyEvent;
import com.game.client.window.event.key.Keys;
...
public class EventFactory {
    ...
    public static KeyEvent buildKeyEvent(int key, int scanCode, int action, int mods) {
        ...
        return new KeyEvent(k, actionType, key, scanCode, action, mods);
    }
...
//                LogUtil.logDebug(String.format("Pressed %s", key));
    ...
    public static MouseButtonEvent buildMouseEvent(int button, int action, int mods, double x, double y) {
        var mouseButtonEvent = new MouseButtonEvent(mouseButton(button), mouseButtonAction(action), mods, x, y);
    ...
    public static ResizeWindowEvent buildWindowResizeEvent(int newWidth, int newHeight) {
        var resizeWindowEvent = new ResizeWindowEvent(newWidth, newHeight);
